...
std::wstring correct_name = L"你的姓名"; // 替换为你的姓名
...
void Server::start() {
  ...
  if (bind(server_socket, (struct sockaddr*)&server_addr, sizeof(server_addr)) == SOCKET_ERROR) {
    std::wcerr << L"Bind failed!" << std::endl;
...
void Server::handle_client(SOCKET client_socket) {
  ...
  std::wstring wrequest(reinterpret_cast<wchar_t*>(buffer), bytes_received / 2); // 每两个字节为一个 wchar_t
...
std::wstring Server::process_request(const std::wstring &request) {
  if (request == L"gettime") {
    ...
    if (comma_pos != std::wstring::npos) {
      ...
      return (student_id == correct_id && name == correct_name) ? L"ok" : L"error";
    } else {
      return L"error";
    }
  }
