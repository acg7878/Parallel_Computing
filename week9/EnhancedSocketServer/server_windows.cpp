...
void Server::start() {
  ...
  if (setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&opt,
    ...
    std::cerr << "Failed to set SO_REUSEADDR with error code: "
  ...
  if (bind(server_socket, (struct sockaddr *)&server_addr,
    ...
    std::wcerr << L"Bind failed with error code: " << error_code << std::endl;
...
void Server::handle_client(SOCKET client_socket) {
  ...
  // 设置接收超时时间为 5 秒
  //没效果
//  DWORD timeout = 5000; // 5 秒 = 5000 毫秒
//  int result = setsockopt(client_socket, SOL_SOCKET, SO_RCVTIMEO,
//                          (const char *)&timeout, sizeof(timeout));
//  if (result == SOCKET_ERROR) {
//    // 设置失败，可以输出错误信息
//    int error_code = WSAGetLastError();
//    std::cerr << "setsockopt failed with error code " << error_code
//              << std::endl;
...
std::wstring Server::process_request(const std::wstring &request) {
  if (request == L"gettime") {
    ...
    if (comma_pos != std::wstring::npos) {
      ...
      return (student_id == correct_id && name == correct_name) ? L"ok"
                                                                : L"error";
    } else {
      return L"error";
    }
